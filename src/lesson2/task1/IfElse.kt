...
fun ageDescription(age: Int): String {
    ...
    else if ((age % 10 == 2 && age != 12) || (age % 10 == 3 && age != 13) || (age % 10 == 4 && age != 14)) "$age года"
    else "$age лет"
}
...
fun timeForHalfWay(t1: Double, v1: Double,
    ...
    val hw = (t1 * v1 + t2 * v2 + t3 * v3) / 2.0
    val s1 = v1 * t1
    val s2 = v2 * t2
    return when {
        hw < s1 -> hw / v1
        hw in s1..(s1 + s2) -> t1 + ((hw - s1) / v2)
        else -> t1 + t2 + ((hw - s1 - s2) / v3)
    }
}
...
fun triangleKind(a: Double, b: Double, c: Double): Int {
    ...
    else return if ((c > a + b) || (b > a + c) || (a > c + b)) -1
    else return if ((c * c > a * a + b * b) || (b * b > a * a + c * c) || (a * a > c * c + b * b)) 2
    else return 0
}
/**
 * Средняя
...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int {
    return if (b in (c + 1)..(d - 1) && a < c) b - c
    else if (d in (a + 1)..(b - 1) && a > c) d - a
    else if (a > c && b < d) b - a
    else if (a < c && b > d) d - c
    else if (b == c || a == d) 0
    else - 1
